...
{
    return mdns_service_txt_set_with_timeout(service, proto, txt, num_items, 0);
}

esp_err_t mdns_service_txt_set_with_timeout(const char * service, const char * proto, mdns_txt_item_t txt[], uint8_t num_items, uint32_t timeout_ms)
{
    if (!_mdns_server || !_mdns_server->services || _str_null_or_empty(service) || _str_null_or_empty(proto) || (num_items && txt == NULL)) {
        return ESP_ERR_INVALID_ARG;
    }
    mdns_srv_item_t * s = _mdns_get_service_item(service, proto, NULL);
    if (!s) {
    ...
    if (xQueueSend(_mdns_server->action_queue, &action, pdMS_TO_TICKS(timeout_ms)) != pdPASS) {
        _mdns_free_linked_txt(new_txt);
        free(action);
        return ESP_ERR_NO_MEM;
    }
